...
class MysqliDb
{
    ...
    /**
     * *** THE MAGIG GETUSER FUNCTION - GET THE ENTIRE ROW OF DATA FROM A USER'S RECORDS.
     *
     * @param it retrieves a row of data for one user making easy to extract individual fields
     *    
     * @param string $userid is the unique id of the user in the table $tableName
     *
     * @return array Contains the returned rows from the select query.
     */
    public function getUser($userid, $tableName)
    {
        ...
        $this->_query = 'SELECT ' . implode(' ', $this->_queryOptions) . ' ' .
            $column . " FROM " . $this->_tableName . " WHERE id=" . $userid;
        $stmt = $this->_buildQuery($numRows);
